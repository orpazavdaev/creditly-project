...
---
## Theoretical Retry + Circuit Breaker Mechanism for CRM Integration 

This section describes a **theoretical** design for a retry mechanism combined with a circuit breaker pattern for the CRM integration layer.
This mechanism is not currently implemented in the project, but represents a production-grade enhancement for improving reliability and fault tolerance when working with external systems.
...
A **background worker** would then process these jobs independently. Additionally, a **circuit breaker** is introduced to prevent repeated attempts when the CRM is unavailable.

Responsibilities Split
- Retry mechanism -
Handles temporary failures by retrying requests over time.
- Circuit breaker -
Detects system-wide failures and temporarily stops outgoing requests to avoid overload.

Together, they ensure both **resilience** and **system protection**.
...
5. Before calling CRM → circuit breaker state is checked
6. Request is sent (or skipped if circuit is open)
7. Retry logic applies if needed
...
- **Closed (normal)** - 
Requests flow normally. Failures are tracked.
- **Open (blocked)** - 
After multiple consecutive failures, requests are stopped immediately (fail fast).
Jobs are not sent to CRM, only rescheduled.
- **Half-Open (test mode)** - 
After a cooldown period, a limited number of requests are allowed:
...
Small randomness is added to avoid synchronized retries
...
- Prevents retry storms when CRM is down
- Protects both your system and the external CRM
- Improves recovery behavior after outages
